	...
	else
	{
			...
			(prv_get_bits(122, 125, (uint32_t*)g_card_info->csd) >= 4))
		{
			...
			if ((status & SD_INT_ERROR) == 0)
			{
				...
				if (((g_card_info->ext_csd[46] >> 8) & 0xFF) == 1)
				{
	...
	while (state < 100)
	{
		switch (state)
		{
			...
			case 10:      /* Setup for MMC */
				...
				g_card_info->card_type &= ~CARD_TYPE_SD;
				g_card_info->card_type |= CARD_TYPE_EMMC;
				status = sdmmc_execute_command(CMD_IDLE, 0, MCI_INT_CMD_DONE);
...
* @return 		TRUE/FALSE 
**********************************************************************/
Bool sdmmc_width_set(uint8_t width)
{
		uint32_t status;
		...
		switch(width)
		{
				case 0x01:
					{
							status = sdmmc_execute_command(CMD_SWITCH, 0x03B70000, 0);  // 修改ECSD[183],设置宽度
							if (status != 0)
								return FALSE;
				...
				case 0x04:
					{
							status = sdmmc_execute_command(CMD_SWITCH, 0x03B70100, 0);  // 修改ECSD[183],设置宽度
							if (status != 0)
								return FALSE;
				...
				case 0x08:
					{
							status = sdmmc_execute_command(CMD_SWITCH, 0x03B70200, 0);  // 修改ECSD[183],设置宽度
							if (status != 0)
								return FALSE;

							/* if positive response */
							LPC_SDMMC->CTYPE = MCI_CTYPE_8BIT;
					}
					break;
				default:
					break;
...
/*********************************************************************//**
* @brief 		set the highspeed
* @param[in]	width  
* @return 		TRUE/FALSE 
**********************************************************************/
Bool sdmmc_highspeed_set(void)
{
		uint32_t status;
	
		status = sdmmc_execute_command(CMD_SWITCH, 0x03B90100, 0);  // 修改ECSD[185],设置高速
 		if (status != 0)
 			return FALSE;
 		prv_process_csd();
