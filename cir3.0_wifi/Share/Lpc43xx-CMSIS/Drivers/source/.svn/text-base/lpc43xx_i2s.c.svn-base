...
void I2S_Config(LPC_I2Sn_Type *I2Sx, uint8_t TRMode, I2S_CFG_Type* ConfigStruct)
{
	...
	config = (ConfigStruct->ws_halfperiod / 2 - 1)<<6 | (ConfigStruct->ws_sel)<<5 | (ConfigStruct->reset)<<4 |
 ...
 * @param[in]	I2Sx I2S peripheral selected, should be: LPC_I2S
 * @param[in]	Freq is the frequency for I2S will be set. It can range
 * 						from 8-96 kHz(8, 16, 22.05, 32, 44.1, 48, 96kHz)
 * @param[in]	TRMode is transmit/receive mode, should be:
 * 					- I2S_TX_MODE = 0	:transmit mode
 * 					- I2S_RX_MODE = 1	:receive mode
 * @return 		Status: ERROR or SUCCESS
 *********************************************************************/
Status I2S_FreqConfig_PLL0(LPC_I2Sn_Type *I2Sx, uint8_t N, uint8_t TRMode) {
	...
	{
		if(I2Sx == LPC_I2S0)
			LPC_CREG->CREG6 |= (1<<12);							/* TX_MCLK is derived from the output of PLL0AUDIO */
		else
			LPC_CREG->CREG6 |= (1<<14);	
	}
	else 											//Receiver
	{
		if(I2Sx == LPC_I2S0)
			LPC_CREG->CREG6 |= (1<<13);							/* RX_MCLK is derived from the output of PLL0AUDIO */
		else
			LPC_CREG->CREG6 |= (1<<15);
	}
		...
		uint8_t TRMode)
{
	...
	if (TRMode == I2S_TX_MODE) {
		...
		if (ModeConfig->clksel == I2S_CLKSEL_PLLAUDIO) {
			I2Sx->TXMODE |= 0x01;
		}
