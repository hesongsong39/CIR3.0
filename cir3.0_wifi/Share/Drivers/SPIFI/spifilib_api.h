...
/**
 * @brief	Erase multiple SubBlocks-Sectors
 * @param	pHandle		: Pointer to a LPCSPIFILIB device handle
 * @param	firstBlock	: First SubBlocks-Sectors number to erase
 * @param	numBlocks	: Number of SubBlocks-Sectors to erase
 * @return	A SPIFI_ERR_xxx error code (SPIFI_ERR_NONE is no errors)
 * @note	If any of the specified params are invalid, the operation is aborted
 */
SPIFI_ERR_T spifiEraseSector(SPIFI_HANDLE_T *pHandle, uint32_t firstSectors, uint32_t numSectors);
