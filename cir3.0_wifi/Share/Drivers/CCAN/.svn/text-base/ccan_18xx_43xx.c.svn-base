...
void configTimming(LPC_CCAN_T *pCCAN,
						uint32_t ClkDiv,
...
{
    uint32_t  div, quanta, segs, seg1, seg2, clk_per_bit, can_sjw;
    
    clk_per_bit = pClk / bitRate;
   
    for (div = 3; div <= 15; div++) {
          ...
          uint32_t MaskBit)
{
    ...
    pData = (uint16_t *) (pMsgObj->data);
		...
		{
				msgCtrl |= CCAN_IF_MCTRL_UMSK | CCAN_IF_MCTRL_RMTEN(remoteFrame) | 
         (pMsgObj->dlc & CCAN_IF_MCTRL_DLC_MSK);
		}
    ...
    /* Mxtd: 0, Mdir: 1, Mask is 0x7FF */
     pCCAN->IF[IFSel].MSK2 = CCAN_IF_MASK2_MDIR(dir) | ((CCAN_MSG_ID_STD_MASK & MaskBit) << 2);
    ...
    pCCAN->IF[IFSel].MSK2 = CCAN_IF_MASK2_MXTD | CCAN_IF_MASK2_MDIR(dir) | ((CCAN_MSG_ID_EXT_MASK & MaskBit) >> 16);
    pCCAN->IF[IFSel].MSK1 = (CCAN_MSG_ID_EXT_MASK & MaskBit) & 0x0000FFFF;
...
void Chip_CCAN_Send(LPC_CCAN_T *pCCAN, CCAN_MSG_IF_T IFSel, Bool remoteFrame, CCAN_MSG_OBJ_T *pMsgObj)
{
    ...
    Chip_CCAN_SetMsgObject(pCCAN, IFSel, CCAN_TX_DIR, remoteFrame,TRUE,msgNum, pMsgObj,0);
...
void Chip_CCAN_AddReceiveID(LPC_CCAN_T *pCCAN, CCAN_MSG_IF_T IFSel, uint32_t id,Bool EOBstate)
{
    ...
    temp.id = id;
    Chip_CCAN_SetMsgObject(pCCAN, IFSel, CCAN_RX_DIR, FALSE, EOBstate ,msgNum, &temp,0);
}
