/************************************************************
  ...
  FileName: crc.c
  Author: zhaoli      
  Description: 10021003-CRC,ff7e-CRC  校验函数,BCH  编解码处理
  Version:       V1.0.0.1  
  Function List:   
  1. CRC  校验
  2. BCH  编解码
...
//CRC16  特征表
const unsigned short g_Crc_table16[]= {
...
const unsigned char g_Crc_table8[] = {
...
{ 
	...
	while (wlen--) 
	{
		temp = (unsigned char)(crc>>8);   			// 以8   位二进制数的形式暂存CRC的高8  位 
		crc <<= 8;                			// 左移8   位
		crc ^= g_Crc_table16[temp ^ (*ptr)];	//高8  位和当前字节相加后再查表求CRC,  再加上以前的CRC
		ptr++;
	}
	return crc;	
}  
...
参    数:　  *ptr	  数据地址
	           len	  长度		
返 回 值:    Bool		成功则返回TRUE  ，失败则返回FALSE	
***************************************************************/
